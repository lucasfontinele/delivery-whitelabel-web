import type { OffersResponse } from 'types/offer';
import type { StoreResponse } from 'types/store';

export const offers: OffersResponse[] = [
  {
    label: 'Promoções',
    maximum_quantity: 1,
    offers: [
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc243',
        title: "Cerveja Fuller's Imperial Porter 500ml",
        description:
          'Inspirada nas Porters do século 19, ela é mais intensa e alcoólica do que as cervejas da época, além de mais encorpada e potente. Refermentada na garrafa, pode servir também para guarda, pois seus aromas e sabores se desenvolvem com tempo.',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: "Cerveja Fuller's Imperial Porter 500ml",
            description:
              'Inspirada nas Porters do século 19, ela é mais intensa e alcoólica do que as cervejas da época, além de mais encorpada e potente. Refermentada na garrafa, pode servir também para guarda, pois seus aromas e sabores se desenvolvem com tempo.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: false,
          },
        ],
        amount: 135.45,
        amount_without_discount: 270.9,
        discount_percentage: 50,
        maximum_quantity: 1,
      },
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc244',
        title: "Cerveja Fuller's Imperial Stout 500ml",
        description:
          "Fabricada no estilo autêntico do clássico do século 18, a edição limitada da Fuller's Imperial Stout é uma experiência deliciosa com uma infusão de botões de rosa que dão origem a sugestões de delícia turca.",
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: "Cerveja Fuller's Imperial Stout 500ml",
            description:
              "Fabricada no estilo autêntico do clássico do século 18, a edição limitada da Fuller's Imperial Stout é uma experiência deliciosa com uma infusão de botões de rosa que dão origem a sugestões de delícia turca.",
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: false,
          },
        ],
        amount: 116.45,
        amount_without_discount: 232.9,
        discount_percentage: 50,
        maximum_quantity: 1,
      },
    ],
  },
  {
    label: 'Cervejas',
    maximum_quantity: 0,
    offers: [
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc245',
        title: 'Cerveja Colorado Appia 350ml',
        description:
          'A palavra Appia vem do latim e significa abelha. O nome perfeito para a primeira cerveja do Brasil a utilizar mel em sua fórmula. Uma combinação exótica que, além do mel das laranjeiras, é feita a partir da melhor cevada, trigos maltados e nossa exclusiva levedura de alta fermentação. Doce, encorpada e refrescante, é perfeita para quem busca novos e diferentes sabores. O seu estilo Honey Wheat Ale, combina com queijo brie, pernil e massas leves. Colorado Appia ganhou por três anos consecutivos a Medalha de bronze no festival Brasileiro de Cerveja. (2013, 2014 e 2015). Campeã Nacional com Medalha de Ouro no Estilo Flavoured Honey & Maple na WBA 2017.',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Cerveja Colorado Appia 350ml',
            description:
              'A palavra Appia vem do latim e significa abelha. O nome perfeito para a primeira cerveja do Brasil a utilizar mel em sua fórmula. Uma combinação exótica que, além do mel das laranjeiras, é feita a partir da melhor cevada, trigos maltados e nossa exclusiva levedura de alta fermentação. Doce, encorpada e refrescante, é perfeita para quem busca novos e diferentes sabores. O seu estilo Honey Wheat Ale, combina com queijo brie, pernil e massas leves. Colorado Appia ganhou por três anos consecutivos a Medalha de bronze no festival Brasileiro de Cerveja. (2013, 2014 e 2015). Campeã Nacional com Medalha de Ouro no Estilo Flavoured Honey & Maple na WBA 2017.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: false,
          },
        ],
        amount: 4.99,
        amount_without_discount: 5.99,
        discount_percentage: 16.6,
        maximum_quantity: 12,
      },
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc246',
        title: 'Cerveja Colorado Ribeirão Lager 600ml',
        description:
          'O lançamento mais esperado da Cervejaria Colorado, a primeira artesanal do Brasil, acabou de chegar. E você, que faz parte do Empório da Cerveja, tem acesso antes de TODO MUNDO! Essa é a Colorado Ribeirão Lager. Uma cerveja que o nome já diz tudo. Nascida em Ribeirão Preto, nossa casa, é uma lager leve e refrescante, mas com a cara de Colorado, uma cervejaria que nunca faria mais do mesmo: cerveja clara com laranja, com IBU 20 e 4,5% de teor alcoólico com toda qualidade da primeira cervejaria artesanal do Brasil por um preço muito mais acessível do que a maioria!',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Cerveja Colorado Ribeirão Lager 600ml',
            description:
              'O lançamento mais esperado da Cervejaria Colorado, a primeira artesanal do Brasil, acabou de chegar. E você, que faz parte do Empório da Cerveja, tem acesso antes de TODO MUNDO! Essa é a Colorado Ribeirão Lager. Uma cerveja que o nome já diz tudo. Nascida em Ribeirão Preto, nossa casa, é uma lager leve e refrescante, mas com a cara de Colorado, uma cervejaria que nunca faria mais do mesmo: cerveja clara com laranja, com IBU 20 e 4,5% de teor alcoólico com toda qualidade da primeira cervejaria artesanal do Brasil por um preço muito mais acessível do que a maioria!',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: false,
          },
        ],
        amount: 12.99,
        amount_without_discount: 20.99,
        discount_percentage: 38.1,
        maximum_quantity: 6,
      },
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc247',
        title: "Cerveja Beck's 600ml",
        description:
          'A Cerveja Becks é uma legítima German Lager Puro Malte que segue a risca a Lei da Pureza da Cerveja desde 1873. Seu IBU (Unidade Internacional de Amargor) é 20. O resultado é uma cerveja de amargor intenso, sabor marcante e excepcional pureza. Ela é provocante, cheia de personalidade com um sabor característico e único. É a German Lager mais vendida no mundo.',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: "Cerveja Beck's 600ml",
            description:
              'A Cerveja Becks é uma legítima German Lager Puro Malte que segue a risca a Lei da Pureza da Cerveja desde 1873. Seu IBU (Unidade Internacional de Amargor) é 20. O resultado é uma cerveja de amargor intenso, sabor marcante e excepcional pureza. Ela é provocante, cheia de personalidade com um sabor característico e único. É a German Lager mais vendida no mundo.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: false,
          },
        ],
        amount: 9.99,
        amount_without_discount: 15.99,
        discount_percentage: 37.5,
        maximum_quantity: 6,
      },
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc248',
        title: 'Cerveja Hocus Pocus Magic Trap Belgian Strong Golden Ale 500ml',
        description:
          'A palavra Appia vem do latim e significa abelha. O nome perfeito para a primeira cerveja do Brasil a utilizar mel em sua fórmula. Uma combinação exótica que, além do mel das laranjeiras, é feita a partir da melhor cevada, trigos maltados e nossa exclusiva levedura de alta fermentação. Doce, encorpada e refrescante, é perfeita para quem busca novos e diferentes sabores. O seu estilo Honey Wheat Ale, combina com queijo brie, pernil e massas leves. Colorado Appia ganhou por três anos consecutivos a Medalha de bronze no festival Brasileiro de Cerveja. (2013, 2014 e 2015). Campeã Nacional com Medalha de Ouro no Estilo Flavoured Honey & Maple na WBA 2017.',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title:
              'Cerveja Hocus Pocus Magic Trap Belgian Strong Golden Ale 500ml',
            description:
              'A palavra Appia vem do latim e significa abelha. O nome perfeito para a primeira cerveja do Brasil a utilizar mel em sua fórmula. Uma combinação exótica que, além do mel das laranjeiras, é feita a partir da melhor cevada, trigos maltados e nossa exclusiva levedura de alta fermentação. Doce, encorpada e refrescante, é perfeita para quem busca novos e diferentes sabores. O seu estilo Honey Wheat Ale, combina com queijo brie, pernil e massas leves. Colorado Appia ganhou por três anos consecutivos a Medalha de bronze no festival Brasileiro de Cerveja. (2013, 2014 e 2015). Campeã Nacional com Medalha de Ouro no Estilo Flavoured Honey & Maple na WBA 2017.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: false,
          },
        ],
        amount: 18.68,
        amount_without_discount: 24.9,
        discount_percentage: 37.5,
        maximum_quantity: 6,
      },
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc249',
        title: 'Combo Colorado',
        description:
          'Kit: 3 Cervejas Colorado Appia 350ml + 3 Cervejas Colorado Ribeirão Lager 600ml',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Cerveja Colorado Appia 350ml',
            description:
              'A palavra Appia vem do latim e significa abelha. O nome perfeito para a primeira cerveja do Brasil a utilizar mel em sua fórmula. Uma combinação exótica que, além do mel das laranjeiras, é feita a partir da melhor cevada, trigos maltados e nossa exclusiva levedura de alta fermentação. Doce, encorpada e refrescante, é perfeita para quem busca novos e diferentes sabores. O seu estilo Honey Wheat Ale, combina com queijo brie, pernil e massas leves. Colorado Appia ganhou por três anos consecutivos a Medalha de bronze no festival Brasileiro de Cerveja. (2013, 2014 e 2015). Campeã Nacional com Medalha de Ouro no Estilo Flavoured Honey & Maple na WBA 2017.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: false,
          },
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Cerveja Colorado Ribeirão Lager 600ml',
            description:
              'O lançamento mais esperado da Cervejaria Colorado, a primeira artesanal do Brasil, acabou de chegar. E você, que faz parte do Empório da Cerveja, tem acesso antes de TODO MUNDO! Essa é a Colorado Ribeirão Lager. Uma cerveja que o nome já diz tudo. Nascida em Ribeirão Preto, nossa casa, é uma lager leve e refrescante, mas com a cara de Colorado, uma cervejaria que nunca faria mais do mesmo: cerveja clara com laranja, com IBU 20 e 4,5% de teor alcoólico com toda qualidade da primeira cervejaria artesanal do Brasil por um preço muito mais acessível do que a maioria!',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: false,
          },
        ],
        amount: 49.99,
        amount_without_discount: 53.94,
        discount_percentage: 7.3,
        maximum_quantity: 3,
      },
    ],
  },
  {
    label: 'Drinks',
    maximum_quantity: 0,
    offers: [
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc253',
        title: 'Negroni',
        description:
          'O Negroni é um drink italiano que foi batizado em homenagem ao conde Camillo Negroni, que em 1919, pediu para que adicionassem um toque de gim em vez de água com gás ao seu drink americano.',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Negroni',
            description:
              'O Negroni é um drink italiano que foi batizado em homenagem ao conde Camillo Negroni, que em 1919, pediu para que adicionassem um toque de gim em vez de água com gás ao seu drink americano.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: true,
          },
        ],
        amount: 18.0,
        amount_without_discount: 30.0,
        discount_percentage: 40,
        maximum_quantity: 12,
      },
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc263',
        title: 'Martini',
        description:
          "O Martini, também conhecido como Dry Martini, é um coquetel feito com gim e vermute seco, na proporção de seis para um, sendo os ingredientes misturados e coados, servidos em um copo gelado. O martíni é um ícone popular, sendo conhecido como o 'rei dos coquetéis'.",
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Martini',
            description:
              "O Martini, também conhecido como Dry Martini, é um coquetel feito com gim e vermute seco, na proporção de seis para um, sendo os ingredientes misturados e coados, servidos em um copo gelado. O martíni é um ícone popular, sendo conhecido como o 'rei dos coquetéis'.",
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: true,
          },
        ],
        amount: 18.0,
        amount_without_discount: 30.0,
        discount_percentage: 40,
        maximum_quantity: 12,
      },
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc273',
        title: 'Whisky Sour',
        description:
          'O Whisky Sour é uma bebida mista que contém uísque, suco de limão e açúcar. Para muitos, porém, faz parte de uma família de coquetéis antigos que lembram meados do século XX. Na verdade, o US Bartenders Guide escolheu o coquetel com sabor de frutas cítricas como representante da década de 20-1960.',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Whisky Sour',
            description:
              'O Whisky Sour é uma bebida mista que contém uísque, suco de limão e açúcar. Para muitos, porém, faz parte de uma família de coquetéis antigos que lembram meados do século XX. Na verdade, o US Bartenders Guide escolheu o coquetel com sabor de frutas cítricas como representante da década de 20-1960.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: true,
          },
        ],
        amount: 18.0,
        amount_without_discount: 30.0,
        discount_percentage: 40,
        maximum_quantity: 12,
      },
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc283',
        title: 'Combo da Noite',
        description: 'Kit: 1 Whisky Sour + 1 Caipirinha',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Whisky Sour',
            description:
              'O Whisky Sour é uma bebida mista que contém uísque, suco de limão e açúcar. Para muitos, porém, faz parte de uma família de coquetéis antigos que lembram meados do século XX. Na verdade, o US Bartenders Guide escolheu o coquetel com sabor de frutas cítricas como representante da década de 20-1960.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: true,
          },
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: ' Caipirinha ',
            description:
              'A caipirinha é uma bebida alcoólica brasileira, ou um coquetel de origem paulista, feita com cachaça, limão, açúcar e gelo.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: true,
          },
        ],
        amount: 30.0,
        amount_without_discount: 36.0,
        discount_percentage: 16.6,
        maximum_quantity: 10,
      },
    ],
  },
  {
    label: 'Petiscos',
    maximum_quantity: 0,
    offers: [
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc293',
        title: 'Combo: 3 Pastéis de Costela + 6 bolinhos de Bacalhau',
        description: '3 unidades de pastéis com recheio de costela.',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Pastéis de Costela',
            description: '3 unidades de pastéis com recheio de costela.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: true,
          },
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Bolinhos de Bacalhau',
            description:
              '6 unidades do bolinho típico da culinária portuguesa.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: true,
          },
        ],
        amount: 24.9,
        amount_without_discount: 29.9,
        discount_percentage: 16,
        maximum_quantity: 4,
      },
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc343',
        title: 'Espetinho de Carne',
        description: '1 unidade de espetinho de coração de alcatra.',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Espetinho de Carne',
            description: '1 unidade de espetinho de coração de alcatra.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: true,
          },
        ],
        amount: 5.0,
        amount_without_discount: 8.0,
        discount_percentage: 37.5,
        maximum_quantity: 10,
      },
    ],
  },
  {
    label: 'Entradas',
    maximum_quantity: 0,
    offers: [
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc543',
        title: 'Amendoim Cozido',
        description: 'Porção de 400g de Amendoim Cozido.',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Amendoim Cozido',
            description: 'Porção de 400g de Amendoim Cozido.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: true,
          },
        ],
        amount: 16.0,
        amount_without_discount: 24.0,
        discount_percentage: 33.3,
        maximum_quantity: 4,
      },
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc643',
        title: 'Feijão Amigo',
        description: 'Porção individual de 200ml de Feijão Amigo.',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Feijão Amigo',
            description: 'Porção individual de 200ml de Feijão Amigo.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: true,
          },
        ],
        amount: 6.0,
        amount_without_discount: 11.0,
        discount_percentage: 45.4,
        maximum_quantity: 8,
      },
    ],
  },
  {
    label: 'Pratos Principais ',
    maximum_quantity: 0,
    offers: [
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc743',
        title: 'Filé Mignon aos 4 queijos',
        description:
          'Suculento Filé Mignon selecionado, com nosso exclusivo molho 4 queijos, preparado na hora.',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Filé Mignon aos 4 queijos',
            description:
              'Suculento Filé Mignon selecionado, com nosso exclusivo molho 4 queijos, preparado na hora.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: true,
          },
        ],
        amount: 79.99,
        amount_without_discount: 109.99,
        discount_percentage: 37.5,
        maximum_quantity: 5,
      },
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc843',
        title: 'Combo: Escondidinho de Carne Assada + Escondidinho de Camarão',
        description:
          'Duas porções individuais, 400g cada, de Econdidinho de Carne Assada e Escondidinho de Camarão.',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Escondidinho de Carne Assada',
            description:
              'Carne Assada ao molho vermelho, purê de batata ou aipim, queijo e batata palha.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: true,
          },
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Escondidinho de Camarão',
            description:
              'Camarão ao molho branco, requeijão, purê de batata ou aipim, queijo e batata palha.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: true,
          },
        ],
        amount: 60.0,
        amount_without_discount: 79.99,
        discount_percentage: 24.9,
        maximum_quantity: 5,
      },
    ],
  },
  {
    label: 'Saladas',
    maximum_quantity: 0,
    offers: [
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc943',
        title: 'Salada Primavera',
        description:
          'Folhas, gergelim, abobrinha e tomate com suco de limão e molho.',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Salada Primavera',
            description:
              'Folhas, gergelim, abobrinha e tomate com suco de limão e molho.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: true,
          },
        ],
        amount: 24.9,
        amount_without_discount: 29.9,
        discount_percentage: 16,
        maximum_quantity: 10,
        offer_configuration: [
          {
            id: '20a7f1c0-d7b9-41e3-9824-86ffb773eba2',
            label: 'Molhos Inclusos',
            minimum_quantity: 1,
            maximum_quantity: 2,
            items: [
              {
                id: '38e77528-7cd8-4990-b215-dd4c98897e59',
                amount: 0,
                product: {
                  id: '862481f5-d348-4f5f-acb0-783b4a3f9f0b',
                  title: 'Confit de Alho',
                  description: '10g',
                  photo: {
                    color: '#000000',
                    image:
                      'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/862481f5-d348-4f5f-acb0-783b4a3f9f0b.jpg',
                  },
                },
              },
              {
                id: 'b836a87c-f21c-4f03-b0af-bb9788080949',
                amount: 0,
                product: {
                  id: '3c19373a-bee0-44cc-b579-223cd2fb334f',
                  title: 'Confit de Pimenta',
                  description: '',
                  photo: {
                    color: '#000000',
                    image:
                      'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/3c19373a-bee0-44cc-b579-223cd2fb334f.jpg',
                  },
                },
              },
            ],
          },
          {
            id: '5ecc09b8-08d9-4082-be6e-57b3dbd53190',
            label: 'Molhos Extras',
            minimum_quantity: 0,
            maximum_quantity: 10,
            items: [
              {
                id: 'bb4eae86-0673-4254-b09a-ce9f98df3636',
                amount: 1.99,
                product: {
                  id: '862481f5-d348-4f5f-acb0-783b4a3f9f0b',
                  title: 'Molho Tártaro',
                  description: '10g',
                  photo: {
                    color: '#000000',
                    image:
                      'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/862481f5-d348-4f5f-acb0-783b4a3f9f0b.jpg',
                  },
                },
              },
              {
                id: 'b836a87c-f21c-4f03-b0af-bb9788080949',
                amount: 1.99,
                product: {
                  id: '3c19373a-bee0-44cc-b579-223cd2fb334f',
                  title: 'Molho Mostarda e Mel',
                  description: '10g',
                  photo: {
                    color: '#000000',
                    image:
                      'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/3c19373a-bee0-44cc-b579-223cd2fb334f.jpg',
                  },
                },
              },
              {
                id: 'b836a87c-f21c-4f03-b0af-bb9788080949',
                amount: 1.99,
                product: {
                  id: '3c19373a-bee0-44cc-b579-223cd2fb334f',
                  title: 'Molho Bearnaise',
                  description: '10g',
                  photo: {
                    color: '#000000',
                    image:
                      'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/3c19373a-bee0-44cc-b579-223cd2fb334f.jpg',
                  },
                },
              },
            ],
          },
        ],
      },
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc253',
        title: 'Salada Caesar',
        description: 'Alface romana, queijo parmesão, croutons e molho Caesar.',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Salada Caesar',
            description:
              'Alface romana, queijo parmesão, croutons e molho Caesar.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: true,
          },
        ],
        amount: 24.9,
        amount_without_discount: 29.9,
        discount_percentage: 16,
        maximum_quantity: 10,
      },
    ],
  },
  {
    label: 'Sobremesas',
    maximum_quantity: 0,
    offers: [
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc263',
        title: 'Cheesecake',
        description: 'Cheesecake com calda de frutas vermelhas ou caramelo.',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Cheesecake',
            description:
              'Porção individual de aproximadamente 300g de Cheesecake com calda de frutas vermelhas ou caramelo.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: false,
          },
        ],
        amount: 13.0,
        amount_without_discount: 18.0,
        discount_percentage: 27.7,
        maximum_quantity: 10,
      },
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc273',
        title: 'Pudim de Leite Condensado',
        description:
          'Porção individual de aproximadamente 300g de Pudim de Leite Condensado.',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Pudim de Leite Condensado',
            description:
              'Porção individual de aproximadamente 300g de Pudim de Leite Condensado.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: false,
          },
        ],
        amount: 13.0,
        amount_without_discount: 18.0,
        discount_percentage: 27.7,
        maximum_quantity: 10,
      },
    ],
  },
  {
    label: 'Combos',
    maximum_quantity: 0,
    offers: [
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc283',
        title: 'Escondidinho de Carne Assada + Salada Primavera',
        description: 'Combo: Escondidinho de Carne Assada + Salada Primavera',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Escondidinho de Carne Assada',
            description:
              'Carne Assada ao molho vermelho, purê de batata ou aipim, queijo e batata palha.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: true,
          },
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Salada Primavera',
            description:
              'Folhas, gergelim, abobrinha e tomate com suco de limão e molho.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: true,
          },
        ],
        amount: 40.0,
        amount_without_discount: 54.9,
        discount_percentage: 27.1,
        maximum_quantity: 5,
      },
      {
        id: '26077117-da7a-4aaa-b8d6-dbae0d9cc293',
        title: 'Filé Mignon aos 4 queijos + 2 Pudins de Leite Condensado',
        description:
          'Combo: Filé Mignon aos 4 queijos + 2 Pudins de Leite Condensado',
        products: [
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Filé Mignon aos 4 queijos',
            description:
              'Suculento Filé Mignon selecionado, com nosso exclusivo molho 4 queijos, preparado na hora.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: true,
          },
          {
            id: 'b0b1533c-fb93-4b43-945c-f0be7c4073e8',
            title: 'Pudim de Leite Condensado',
            description:
              'Porção individual de aproximadamente 300g de Pudim de Leite Condensado.',
            photos: [
              {
                color: '#000000',
                image:
                  'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/products/b0b1533c-fb93-4b43-945c-f0be7c4073e8.jpg',
              },
            ],
            need_kitchen: false,
          },
        ],
        amount: 89.99,
        amount_without_discount: 105.99,
        discount_percentage: 15,
        maximum_quantity: 5,
      },
    ],
  },
];

export const store: StoreResponse = {
  id: 'c79a1f90-da57-4358-8c84-1910dea11310',
  profile: {
    name: 'Wäls Gastropub',
    document_number: '134.886.987-00',
    phone_number: '+55 21 97618-7554',
    about:
      'Gastronomia de qualidade e bebidas variadas. Venha conferir! Atenção: Todos os alimentos são artesanais e isentos de corantes e conservantes.',
    cover: {
      color: '#000000',
      image:
        'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/cover.jpg',
    },
    logo: {
      color: '#000000',
      image:
        'https://img.ecommerce.macetado.com/merchants/c79a1f90-da57-4358-8c84-1910dea11310/logo.jpg',
    },
    address: {
      line1: 'Estrada Intendente Magalhães, 4456',
      line2: 'Vila Valqueire - Rio de Janeiro, RJ',
      reference: 'Em frente à Praça Valqueire',
      zip_code: '21330-570',
      location: {
        latitude: '-22.9865387',
        longitude: '-43.2104753',
      },
    },
  },
  order_settings: {
    currency: 'BRL',
    pay_in_app: {
      active: true,
      maximum_order_value: 199.99,
      payment_methods: {
        credit: [
          'mastercard',
          'visa',
          'amex',
          'elo',
          'diners',
          'cabal',
          'banescard',
          'hipercard',
          'discover',
          'aura',
          'jcb',
        ],
        debit: ['mastercard', 'visa-electron', 'elo'],
        meal_voucher: ['alelo', 'ben', 'sodexo', 'ticket', 'vr'],
        food_voucher: ['sodexo', 'vr'],
        transfer: ['pix', 'wallet'],
      },
    },
    pay_on_delivery: {
      active: true,
      maximum_order_value: 399.99,
      payment_methods: {
        credit: [
          'mastercard',
          'visa',
          'amex',
          'elo',
          'diners',
          'cabal',
          'banescard',
          'hipercard',
        ],
        debit: ['mastercard', 'visa-electron', 'elo'],
        meal_voucher: ['alelo', 'ben', 'sodexo', 'ticket', 'vr'],
        food_voucher: ['sodexo', 'vr'],
        transfer: ['pix', 'wallet'],
      },
    },
    takeaway: {
      estimated_time: 30,
    },
    delivery: {
      estimated_time: 60,
      cover_area: [
        {
          delivery_fee: 3.99,
          free_delivery_from: 500.0,
          minimum_order_value: 119.99,
          geometry: {
            type: 'Polygon',
            coordinates: [
              [
                [0.0, 0.0],
                [0.0, 5.0],
                [5.0, 5.0],
                [5.0, 0.0],
              ],
            ],
          },
        },
        {
          delivery_fee: 1.99,
          free_delivery_from: 50.0,
          minimum_order_value: 10.99,
          geometry: {
            type: 'Polygon',
            coordinates: [
              [
                [0.0, 0.0],
                [0.0, 5.0],
                [5.0, 5.0],
                [5.0, 0.0],
              ],
            ],
          },
        },
      ],
    },
  },
  opening_schedule: {
    status: 'auto',
    opening_hours: [
      {
        weekdays: [2, 3, 4, 5, 6],
        delivery: {
          opening: '18:00',
          closing: '22:00',
        },
        takeaway: {
          opening: '18:00',
          closing: '00:00',
        },
        kitchen: {
          opening: '18:00',
          closing: '23:00',
        },
      },
      {
        weekdays: [1, 7],
        delivery: {
          opening: '18:00',
          closing: '21:00',
        },
        takeaway: {
          opening: '18:00',
          closing: '21:00',
        },
        kitchen: {
          opening: '18:00',
          closing: '21:00',
        },
      },
      {
        weekdays: [7],
        delivery: {
          opening: '11:00',
          closing: '13:00',
        },
        takeaway: {
          opening: '11:00',
          closing: '13:00',
        },
        kitchen: {
          opening: '11:00',
          closing: '13:00',
        },
      },
    ],
    opening_days: [
      {
        day: '2021-02-25',
        delivery: {
          opening: '11:00',
          closing: '13:00',
        },
        takeaway: {
          opening: '11:00',
          closing: '13:00',
        },
        kitchen: {
          opening: '11:00',
          closing: '13:00',
        },
      },
      {
        day: '2021-02-25',
        delivery: {
          opening: '18:00',
          closing: '21:00',
        },
        takeaway: {
          opening: '18:00',
          closing: '21:00',
        },
        kitchen: {
          opening: '18:00',
          closing: '21:00',
        },
      },
    ],
    closed_days: [
      '2021-02-15',
      '2021-02-16',
      '2021-02-17',
      '2021-03-01',
      // '2022-09-25',
      // '2022-09-27',
    ],
  },
};

// Configuração de pedidos
